...
import { Modal, Form, Input, Button, message, Row, Col, DatePicker, InputNumber, Space, Typography, Table } from "antd";
import { PlusOutlined, DeleteOutlined } from '@ant-design/icons';
import { registerOrder, modifyOrder } from "../../api/sales/orderApi";
...
const OrderModal = ({ open, onClose, orderData, onRefresh }) => {
  ...
  useEffect(() => {
    if (open) {
      if (isEditing) {
        const formData = {
          ...orderData,
          orderDueDate: orderData.orderDueDate ? dayjs(orderData.orderDueDate) : null,
          orderDate: orderData.orderDate ? dayjs(orderData.orderDate).format('YYYY-MM-DD') : null,
          writerInfo: `${orderData.writer} (${orderData.userId})`,
        ...
        form.setFieldsValue({
          ...
          orderDate: today.format('YYYY-MM-DD'),
    ...
    },
    {
      ...
      render: (_, field) => (
        <Form.Item name={[field.name, 'unitPrice']} rules={[{ required: true, type: 'number', min: 0, message: '0 이상' }]} style={{ marginBottom: 0 }}>
          <InputNumber style={{ width: '100%', textAlign: 'right' }} placeholder="공급가액" formatter={(v) => `${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} parser={(v) => v.replace(/\$\s?|(,*)/g, "")} />
        </Form.Item>
      ),
    },
    {
      ...
      render: (_, field) => (
        <Form.Item name={[field.name, 'unitVat']} rules={[{ required: true, type: 'number', min: 0, message: '0 이상' }]} style={{ marginBottom: 0 }}>
          <InputNumber style={{ width: '100%', textAlign: 'right' }} placeholder="부가세액" formatter={(v) => `${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} parser={(v) => v.replace(/\$\s?|(,*)/g, "")} />
        </Form.Item>
      ),
    },
    {
  ...
  return (
    ...
    >
      ...
      <Form form={form} layout="vertical" onFinish={handleFinish} onValuesChange={handleValuesChange} style={{ marginTop: 24 }}>
        <Row gutter={16}>
          <Col span={6}><Form.Item name="orderId" label="주문 번호"><Input disabled /></Form.Item></Col>
          <Col span={6}></Col>
          <Col span={6}><Form.Item name="writerInfo" label="담당자"><Input disabled /></Form.Item></Col>
          <Col span={6}><Form.Item name="orderStatus" label="주문 상태"><Input disabled /></Form.Item></Col>
        </Row>
        <Row gutter={16}>
          <Col span={6}><Form.Item name="clientId" label="사업자번호" rules={[{ required: true, message: "거래처 사업자등록 번호"}]}><Input placeholder="거래처 사업자등록번호 입력"/></Form.Item></Col>
          <Col span={6}><Form.Item name="clientCompany" label="거래처명" rules={[{ required: true, message: "거래처명"}]}><Input /></Form.Item></Col>
         ...
         <Row gutter={16}>
          <Col span={12}><Form.Item name="orderDueDate" label="납기 예정일"><DatePicker style={{ width: "100%" }} format="YYYY-MM-DD"/></Form.Item></Col>
          <Col span={12}>
        ...
        <Form.Item name="orderNote" label="주문 비고"><Input.TextArea rows={3} /></Form.Item>

        <Typography.Title level={5} style={{ marginTop: 24, marginBottom: 16 }}>주문 항목</Typography.Title>
